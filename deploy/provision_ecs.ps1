...
# Prerequisites:
#   1. Alibaba Cloud CLI installed and on PATH (see docs/DEPLOY_ALIBABA_ECS.md §1b)
...
# ============ CONFIG ============
$Region          = 'ap-southeast-1'          # Singapore. Change to cn-hongkong if preferred.
$InstanceType    = 'ecs.e-c1m2.large'        # 2 vCPU / 4 GB. Fallback: ecs.u1-c1m2.large
$ImageId         = 'ubuntu_22_04_x64_20G_alibase_20240220.vhd'  # verify with `aliyun ecs DescribeImages`
$SystemDiskSize  = 40                        # GB
...
$Bandwidth       = 5                         # Mbps peak, pay-by-traffic
# ================================
...
if ($LASTEXITCODE -ne 0) { throw "aliyun CLI not on PATH. See DEPLOY_ALIBABA_ECS.md §1b." }
Say "aliyun $ver"

# Get your public IP so we can lock down SSH to it.
try { $MyIp = (Invoke-RestMethod https://api.ipify.org).Trim() } catch { $MyIp = '0.0.0.0/0' }
