...
# --- 4. Key pair ---
# If local pem exists we reuse it; otherwise generate a timestamped name so we
# never collide with a stale server-side key from a previous failed run.
if (Test-Path $KeyOutPath) {
    Say "Reusing existing key pair ($KeyPairName) -> $KeyOutPath"
} else {
    $KeyPairName = 'maindesk-key-' + (Get-Date -Format 'yyyyMMddHHmmss')
    $KeyOutPath  = "$HOME\.ssh\$KeyPairName.pem"
    Say "Creating fresh key pair ($KeyPairName) -> $KeyOutPath"
...
$PublicIp = $null
for ($i=0; $i -lt 60; $i++) {
    # DescribeInstanceAttribute takes InstanceId singular (no JSON array).
    $inst = Ali @('ecs','DescribeInstanceAttribute','--InstanceId',$InstanceId)
